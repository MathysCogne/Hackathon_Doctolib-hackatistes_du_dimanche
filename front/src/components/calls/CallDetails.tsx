...
// Sub-components
const CallHeader = ({ priority, time, checked }: { priority: Call['priority']; time: string; checked: boolean }) => (
  <div className="flex items-center justify-between">
    <div className="flex items-center gap-4">
      ...
      <CallStatusIndicator isActive={!checked} />
    </div>
    <span className="text-lg font-semibold text-gray-700">{time}</span>
  </div>
);

const CallStatusIndicator = ({ isActive }: { isActive: boolean }) => (
  <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full shadow-sm ${
    ...
    <span className={`text-sm font-semibold ${
      ...
      {isActive ? "Active Call" : "Disconnected"}
    </span>
  </div>
);
  ...
  return (
    <DialogContent className="max-w-4xl bg-white/95 backdrop-blur-xl p-6 shadow-lg border border-gray-200">
      <DialogHeader className="border-b border-gray-200 pb-4">
        <CallHeader priority={call.priority} time={call.dateTime} checked={call.checked} />
